<x-guest-layout>
    <div class="max-w-6xl mx-auto mt-1 py-6 px-3">
        <div class="container">
            <nav class="flex mt-0">
                <ol role="list" class="flex items-center space-x-0.5">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <div class="-m-1">
                                <a href="{{route('course.show', $course->slug )}}" class="p-1 ml-0.5 text-base font-medium text-gray-400 rounded-md focus:outline-none focus:ring-2 focus:text-gray-900 focus:ring-gray-900 hover:text-gray-700"> Courses </a>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div class="flex items-center">
                            ...
                            <div class="-m-1">
                                <a href="{{route('course.show', $course->slug)}}" class="p-1 ml-0.5 text-base font-medium text-gray-400 rounded-md focus:outline-none focus:ring-2 focus:text-gray-900 focus:ring-gray-900 hover:text-gray-700">
                                    {{$course->name}} </a>
                            </div>
                        </div>
                    </li>
                ...
                <div class="mt-2 w-2/3 border-gray-100 border-r pr-10 pb-0 border-b-0">
                    ...
                    <ul class="flex flex-wrap items-center gap-4 mt-5 text-gray-700">
                        <li class="flex items-center gap-2">
                            ...
                            <span>{{$course->convertDuration($course->duration)}}</span>
                        </li>
                        <li class="flex items-center gap-2">
                            <svg class="w-6 h-6 text-gray-400 shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z"></path>
                            </svg>
                            <span>{{$course->convertDifficultyLevel($course->difficulty_level)}}</span>
                        </li>
                        <li class="flex items-center gap-2">
                            <svg class="w-6 h-6 text-gray-400 shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        ...
                        @if(count($course->reviews) > 0)
                            @foreach ($course->reviews as  $review)
                                <div class="bg-white p-5 mb-6">
                                    <div>
                                        <div class="py-2">{{ $review->comment }}</div>
                                        <h2 class="font-bold py-2">{{ $review->user->name }} </h2>
                                    </div>
                                </div>
                            @endforeach
                        @else
                        ...
                        <div class="space-y-4 p-6">
                            ...
                            <div class="space-y-4 pb-2">
                                <div class="aspect-w-3 aspect-h-2">
                                    {{-- <img class="rounded-tl rounded-tr" src="{{ asset('assets/images/courses/') }}/{{ $course->image }}.png" alt="{{ $author->name }}"/> --}}
                                    <img class="object-cover w-48 m-auto shadow rounded-full" src="{{ asset('assets/images/authors/') }}/{{ $author->image }}.png" alt="{{ $author->name }}">
                                </div>
                                ...
                                <ul role="list" class="flex space-x-5 justify-center">
                                    <li>
                                        <a href="{{ $author->twitter }}" class="text-gray-400 hover:text-gray-500" target="_blank">
                                    ...
                                    <li>
                                        <a href="{{ $author->github }}" class="text-gray-400 hover:text-gray-500" target="_blank">
                                            <span class="sr-only">{{ $author->github }}</span>
                                    ...
                                    <li>
                                        <a href="{{ $author->facebook }}" class="text-gray-400 hover:text-gray-500" target="_blank">
                                ...
                                <div class="text-sm px-7">
                                    <p class="text-gray-500 text-justify">{{ $author->details }}</p>
                                </div>
                            </div>
                            @endforeach
                        </div>
